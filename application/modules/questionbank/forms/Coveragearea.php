 ...
 * 
 */
class Questionbank_Form_Coveragearea extends Zend_Form
{
    ...
    public function init()
    {
        ...
        $coverage_area->setLabel('Please select coverage area')
                          ->setAttrib('class', 'selectpicker form-control')
                          ->removeDecorator("errors")
                ->setOptions(array('multiple' => FALSE))
                ->setRegisterInArrayValidator(FALSE)
                ->setRequired(true)
                ->addMultiOptions(array(''=>'Please Select','Php'=>'Php','Java'=>'Java','AS400' => 'AS400'));
        $this->addElement($coverage_area);

        $coverage_level = new Zend_Form_Element_Select('coverage_level');
        $coverage_level->setLabel('Please select level')
                          ->setAttrib('class', 'dateDiv selectpicker form-control')
                          ->removeDecorator("errors")
                ->setOptions(array('multiple' => FALSE))
                ->setRegisterInArrayValidator(FALSE)
                ->setRequired(true)
                ->addMultiOptions(array(''=>'Please Select','L1'=>'L1','L2'=>'L2'));
        $this->addElement($coverage_level);


        $input_type = new Zend_Form_Element_Select('input_type');
        $input_type->setLabel('Please select type of question')
                          ->setAttrib('class', 'dateDiv selectpicker form-control')
                          ->removeDecorator("errors")
                ->setOptions(array('multiple' => FALSE))
                ->setRegisterInArrayValidator(FALSE)
                ->setRequired(true)
                ->addMultiOptions(array(''=>'Please Select','radio'=>'Radio','textarea'=>'Text','checkbox' => 'Checkbox','label' => 'Label', 'date' => 'Date'));
		...
		$weightage = new Zend_Form_Element_Text('weightage');
		$weightage->setLabel('Question Weightage')->setRequired(false);
		$this->addElement($weightage);
    
        $max_user = new Zend_Form_Element_Text('max_user');
		$max_user->setLabel('Max Response Allowed')->setRequired(false);
		$this->addElement($max_user);
